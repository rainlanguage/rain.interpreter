...
After any source change affecting bytecode: run `nix develop -c i9r-prelude` → `nix develop -c forge script --silent ./script/BuildPointers.sol` → `nix develop -c forge fmt`, then run `nix develop -c forge test --match-contract LibInterpreterDeployTest` to get new deploy addresses/codehashes. Update `LibInterpreterDeploy.sol` and repeat until stable (constants cascade through the deploy chain).
