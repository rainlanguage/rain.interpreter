...
use ethers::{providers::{Http, Provider}, types::H160, abi::{ParamType, Token}};
use revm::primitives::Address; 
use std::sync::Arc;
use std::str::FromStr;
use crate::interpreter::registry::IInterpreterV2; 
use crate::interpreter::deployer::{get_sip_addresses, deploy_expression};
...
pub async fn compute_eval2(
    ...
    let (store,interpreter,parser) = get_sip_addresses(raininterpreter_deployer_npe2_address,client.clone()).await? ;
    ...
    let _ = write_account_info(&mut cache_db,raininterpreter_deployer_npe2_address,arc_client.clone()).await;
    ...
    let encode_dispatch = ethers::types::U256::from_dec_str("4501898752799015415754650127604722314397366187185745756159").unwrap();
    ...
    let inputs: Vec<ethers::types::U256> = vec![
        ethers::types::U256::from_dec_str("123456").unwrap()
    ];   
...
pub async fn eval_expression( 
    ...
    let rain_interpreter = IInterpreterV2::new(H160::from_str(&interpreter.to_string())?, client.clone()); 

    let eval_tx = rain_interpreter.eval_2(H160::from_str(&store.to_string())?, statenamespace, encode_dispatch, context,inputs);  
