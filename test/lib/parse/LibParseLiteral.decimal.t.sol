...
contract LibParseLiteralDecimalTest is Test {
    ...
    function testParseLiteralDecimalEmpty() external {
        bytes memory data = "";
        (uint256 value) = LibParseLiteral.parseLiteralDecimal(data, Pointer.unwrap(data.dataPointer()), Pointer.unwrap(data.endDataPointer()));
        assertEq(value, 0);
    }
    ...
    function testParseLiteralDecimalSingleDigitE0() external {
        ...
        data = "1e0";
        (value) = LibParseLiteral.parseLiteralDecimal(data, Pointer.unwrap(data.dataPointer()), Pointer.unwrap(data.endDataPointer()));
        assertEq(value, 1);
