...
contract LibOpStackNPTest is RainterpreterExpressionDeployerDeploymentTest {
    ...
    // function testOpStackNPIntegrity(Operand operand, uint8 stackHeight) external {
    //     vm.assume(stackHeight > 0);
    //     function(IntegrityCheckStateNP memory, Operand)
    //         view
    //         returns (uint256, uint256)[] memory integrityCheckers =
    //             new function(IntegrityCheckStateNP memory, Operand) view returns (uint256, uint256)[](1);
    //     integrityCheckers[0] = LibOpStack.integrity;

    //     operand = Operand.wrap(bound(Operand.unwrap(operand), 0, stackHeight - 1));

    //     IntegrityCheckStateNP memory state =
    //         LibIntegrityCheckNP.newState(new bytes[](0), new uint256[](0), integrityCheckers);
    ...
    // function testOpStackIntegrityNPOOBStack(Operand operand, uint8 stackHeight) external {
    //     function(IntegrityCheckStateNP memory, Operand)
    //         view
    //         returns (uint256, uint256)[] memory integrityCheckers =
    //             new function(IntegrityCheckStateNP memory, Operand) view returns (uint256, uint256)[](1);
    ...
    //     Pointer stackBottomBefore = state.stackBottom;
    //     Pointer stackTop = state.stackBottom.unsafeAddWords(stackHeight);
    //     Pointer stackTopAfter = LibOpStackNP.run(state, operand, stackTop);
