...
contract LibOpIfNPTest is OpTest {
    ...
    function testOpIfNPEval3InputsFirstZeroSecondOneThirdTwo() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(0 1 2);");
    ...
    function testOpIfNPEval3InputsFirstOneSecondTwoThirdThree() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(1 2 3);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
        (IInterpreterV1 interpreterDeployer, IInterpreterStoreV1 storeDeployer, address expression) =
            iDeployer.deployExpression(bytecode, constants, minOutputs);
    ...
    function testOpIfNPEval3InputsFirstZeroSecondZeroThirdThree() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(0 0 3);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
        (IInterpreterV1 interpreterDeployer, IInterpreterStoreV1 storeDeployer, address expression) =
            iDeployer.deployExpression(bytecode, constants, minOutputs);
    ...
    function testOpIfNPEval3InputsFirstOneSecondZeroThirdThree() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(1 0 3);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
        (IInterpreterV1 interpreterDeployer, IInterpreterStoreV1 storeDeployer, address expression) =
            iDeployer.deployExpression(bytecode, constants, minOutputs);
    ...
    function testOpIfNPEval3InputsFirstZeroSecondOneThirdZero() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(0 1 0);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
        (IInterpreterV1 interpreterDeployer, IInterpreterStoreV1 storeDeployer, address expression) =
            iDeployer.deployExpression(bytecode, constants, minOutputs);
    ...
    function testOpIfNPEval3InputsFirstZeroSecondZeroThirdOne() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(0 0 1);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
        (IInterpreterV1 interpreterDeployer, IInterpreterStoreV1 storeDeployer, address expression) =
            iDeployer.deployExpression(bytecode, constants, minOutputs);
    ...
    function testOpIfNPEval3InputsFirstTwoSecondThreeThirdFour() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(2 3 4);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
        (IInterpreterV1 interpreterDeployer, IInterpreterStoreV1 storeDeployer, address expression) =
            iDeployer.deployExpression(bytecode, constants, minOutputs);
    ...
    function testOpIfNPEval3InputsFirstTwoSecondZeroThirdFour() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(2 0 4);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
        (IInterpreterV1 interpreterDeployer, IInterpreterStoreV1 storeDeployer, address expression) =
            iDeployer.deployExpression(bytecode, constants, minOutputs);
    ...
    function testOpIfNPEvalFail0Inputs() public {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if();");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
    ...
    function testOpIfNPEvalFail1Input() public {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(0x00);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
    ...
    function testOpIfNPEvalFail2Inputs() public {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(0x00 0x00);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
    ...
    function testOpIfNPEvalFail4Inputs() public {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("_: if(0x00 0x00 0x00 0x00);");
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 1;
