...
contract LibOpCallNPTest is OpTest {
    ...
    function testOpCallNPRunOrdering() external {
        (bytes memory bytecode, uint256[] memory constants) = iDeployer.parse("a b: call<1 2>(10 5); ten five:, a b: int-div(ten five) 9;");
        // The second source is for internal calls only, it is not an entrypoint.
        uint256[] memory minOutputs = new uint256[](1);
        minOutputs[0] = 2;
        (IInterpreterV1 interpreterDeployer, IInterpreterStoreV1 storeDeployer, address expression) = iDeployer.deployExpression(bytecode, constants, minOutputs);
