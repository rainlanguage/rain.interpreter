 ...
 */
export function getBytecodeMeta(bytecode: Bytes): DeployerBytecodeMetaV1 {
  const encoder = new CBOREncoder();
  encoder.addObject(3);

  // -- Add key 0
  encoder.addUint8(0);
  encoder.addBytes(bytecode);

  // -- Add key 1
  // Magic number. This is hardcoded based on the rain metadata spec
  encoder.addUint8(1);
  encoder.addUint64(
    hexToBigInt(MagicNumbers.EXPRESSION_DEPLOYER_V2_BYTECODE_V1).toU64()
  );
  ...
  if (!bytecodeMeta) {
    ...
    bytecodeMeta.magicNumber = hexToBigInt(
      MagicNumbers.EXPRESSION_DEPLOYER_V2_BYTECODE_V1
    );
    bytecodeMeta.payload = bytecode;
